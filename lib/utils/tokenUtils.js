...
export const setupTokenExpiryCheck = (onExpiry) => {
  const token = (typeof window !== 'undefined')
    ? (localStorage.getItem('lsg_access_token') ?? sessionStorage.getItem('lsg_access_token'))
    : null;
  if (!token) return;
...
export const clearAuthData = () => {
  if (typeof window !== 'undefined') {
    ...
    sessionStorage.removeItem('lsg_access_token');
    sessionStorage.removeItem('lsg_refresh_token');
    sessionStorage.removeItem('lsg_user_data');
  }
