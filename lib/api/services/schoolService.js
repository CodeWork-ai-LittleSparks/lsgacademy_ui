...
function buildListKey(params = {}) {
  ...
  return `page=${page}&limit=${limit}&search=${search}&location=${location}&status=${status}&sort_by=${sort_by}&sort_order=${sort_order}`;
}
...
export async function getSchools(params = {}) {
  try {
    ...
    const response = await apiClient.get('/schools', { params: {
      ...
      // API expects `status` as 'active' | 'inactive'. Omit when 'all'.
