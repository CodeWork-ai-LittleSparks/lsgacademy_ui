...
export const createAnnouncement = async (data) => {
  const res = await apiClient.post("/messages/announcements/", data);
  return res.data || res;
};

export const getAnnouncements = async (params) => {
  const res = await apiClient.get("/messages/announcements", { params });
  return res.data || res;
};

export const getAnnouncement = async (announcementId) => {
  const res = await apiClient.get(`/messages/announcements/${announcementId}`);
  return res.data || res;
};

export const markAnnouncementRead = async (announcementId) => {
  await apiClient.put(`/messages/announcements/${announcementId}/read`);
};
