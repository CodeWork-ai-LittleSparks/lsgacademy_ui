...
export const useWebSocket = () => {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        try { ws.send(JSON.stringify({ type: "presence_online" })); } catch {}
    ...
    if (wsRef.current) {
      try {
        if (wsRef.current.readyState === WebSocket.OPEN) {
          wsRef.current.send(JSON.stringify({ type: "presence_offline" }));
        }
      } catch {}
      wsRef.current.close();
      wsRef.current = null;
  ...
  return { isConnected, lastMessage, onlineUsers, isUserOnline, sendMessage, reconnect: connect, disconnect };
};