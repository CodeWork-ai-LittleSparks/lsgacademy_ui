    ...
    <div className="p-4 sm:p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="p-2.5 bg-gradient-to-br from-amber-100 to-orange-100 rounded-xl shadow-sm">
            <BookOpen className="w-6 h-6 text-orange-600" strokeWidth={2.5} />
          </div>
          <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex-1 relative">
          <div className="absolute left-3 top-1/2 -translate-y-1/2 p-1.5 bg-amber-100 rounded-lg">
            <Search className="text-orange-600 h-4 w-4" strokeWidth={2.5} />
          </div>
          <Input 
            value={search} 
            onChange={(e) => setSearch(e.target.value)} 
            placeholder="Search programs by name..." 
            className="w-full pl-12 pr-4 py-3 border-2 border-gray-200 rounded-xl focus:ring-4 focus:ring-orange-100 focus:border-orange-500 font-medium transition-all duration-200 hover:border-orange-300"
          />
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className={`absolute top-0 left-0 right-0 h-1 transition-opacity duration-300 ${
                    p.is_active
                      ? 'bg-gradient-to-r from-amber-500 to-orange-500 opacity-0 group-hover:opacity-100'
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-start gap-3 mb-3">
                          <div className={`flex-shrink-0 p-2.5 rounded-xl shadow-sm ${
                            ...
                            <BookOpen className={`w-5 h-5 ${
                              p.is_active ? 'text-orange-600' : 'text-gray-500'
                            }`} strokeWidth={2.5} />
                          </div>
                          <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-xs font-semibold">
                          {p?.category?.name && (
                            <div className="flex items-center gap-1.5 px-2.5 py-1 bg-orange-100 text-orange-700 rounded-lg">
                              <Tag className="w-3.5 h-3.5" strokeWidth={2.5} />
                              <span>{p.category.name}</span>
                            </div>
                          )}
                          <div className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-100 text-orange-700 rounded-lg">
                      ...
                      <Button 
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-700 hover:to-orange-700 font-semibold shadow-md hover:shadow-lg transition-all duration-200 hover:scale-110 whitespace-nowrap"
                      >
